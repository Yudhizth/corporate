...
$query = "SELECT kode_request FROM tb_kerjasama_perusahan WHERE nomor_kontrak = :nomor";
$stmt = $config->runQuery($query);
$stmt->execute(array(':nomor' => $id));

$row = $stmt->fetch(PDO::FETCH_LAZY);
$type = substr($row['kode_request'], 0, 3);
...
if($type == 'MPO'){ 
	...
	year(curdate()) - year(str_to_date(tb_karyawan.tgl_lahir,'%d-%m-%Y')) as Age, tb_jenis_pekerjaan.nama_pekerjaan FROM tb_kerjasama_perusahan
	...
	while ($mv = $dt->fetch(PDO::FETCH_LAZY)) {
		...
		if ($mv['kode_pekerjaan'] !='') {
			# code...
			$flag = '<label class="label label-lg success">'.$mv['nama_pekerjaan'].'</label>';
		} else {
